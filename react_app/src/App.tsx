...
import { Mode4DecayTest } from './components/Mode4DecayTest';
...
import { Brain, RefreshCw, Zap, Key, X, Loader2, Sparkles } from 'lucide-react';
...
export default function App() {
  ...
  const handlePriorTestComplete = (
    ...
    // Automatically redirect to Tab 1 (Dashboard) upon Phase 4 completion
    setActiveMode(1);
    ...
    <div>
      ...
      <header style={{
        ...
        <div className="app-container" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 1.5rem' }}>
          ...
          {/* Top Left: Logo & Cheat Code Box */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', cursor: 'pointer' }} onClick={handleReset}>
              <div style={{ background: 'linear-gradient(135deg, var(--color-primary), var(--color-accent))', width: '30px', height: '30px', borderRadius: '7px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Brain size={16} color="#fff" />
              </div>
              <span style={{ fontFamily: 'var(--font-heading)', fontWeight: 900, fontSize: '1.15rem', letterSpacing: '-0.02em', background: 'linear-gradient(to right, #fff, #cbd5e1)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
                ANTIGRAVITY
              </span>
            </div>
      ...
      <main className="app-container" style={{ padding: '0 1.5rem', minHeight: 'calc(100vh - 200px)' }}>
        ...
        {/* Tab 5: Learning & Decay Rate Testing */}
        {activeMode === 5 && (
          <Mode4DecayTest onComplete={handleDecayTestComplete} />
        )}
